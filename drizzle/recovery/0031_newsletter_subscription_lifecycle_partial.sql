...
SET @settleclt_0031_column_fingerprint_count = (
  ...
  WHERE TABLE_SCHEMA = DATABASE()
    ...
    AND (
      (COLUMN_NAME = 'source' AND COLUMN_TYPE = 'varchar(64)' AND IS_NULLABLE = 'NO' AND COLUMN_DEFAULT = 'homepage') OR
      (COLUMN_NAME = 'status' AND COLUMN_TYPE = 'enum(''pending'',''active'',''unsubscribed'',''bounced'',''complained'')' AND IS_NULLABLE = 'NO' AND COLUMN_DEFAULT = 'pending') OR
      (COLUMN_NAME = 'consentVersion' AND COLUMN_TYPE = 'varchar(32)' AND IS_NULLABLE IN ('YES', 'NO') AND COLUMN_DEFAULT IS NULL) OR
      (COLUMN_NAME = 'consentedAt' AND DATA_TYPE = 'timestamp' AND IS_NULLABLE IN ('YES', 'NO') AND COLUMN_DEFAULT IS NULL) OR
      (COLUMN_NAME = 'confirmationTokenHash' AND COLUMN_TYPE = 'varchar(64)' AND IS_NULLABLE = 'YES' AND COLUMN_DEFAULT IS NULL) OR
      (COLUMN_NAME = 'confirmationExpiresAt' AND DATA_TYPE = 'timestamp' AND IS_NULLABLE = 'YES' AND COLUMN_DEFAULT IS NULL) OR
      (COLUMN_NAME = 'confirmationSentAt' AND DATA_TYPE = 'timestamp' AND IS_NULLABLE = 'YES' AND COLUMN_DEFAULT IS NULL) OR
      (COLUMN_NAME = 'unsubscribeTokenHash' AND COLUMN_TYPE = 'varchar(64)' AND IS_NULLABLE IN ('YES', 'NO') AND COLUMN_DEFAULT IS NULL) OR
      (COLUMN_NAME = 'confirmedAt' AND DATA_TYPE = 'timestamp' AND IS_NULLABLE = 'YES' AND COLUMN_DEFAULT IS NULL) OR
      (COLUMN_NAME = 'unsubscribedAt' AND DATA_TYPE = 'timestamp' AND IS_NULLABLE = 'YES' AND COLUMN_DEFAULT IS NULL) OR
      (COLUMN_NAME = 'updatedAt' AND DATA_TYPE = 'timestamp' AND IS_NULLABLE = 'NO' AND UPPER(COLUMN_DEFAULT) IN ('CURRENT_TIMESTAMP', 'CURRENT_TIMESTAMP()') AND LOWER(EXTRA) LIKE '%on update current_timestamp%')
    )
);
...
SET @settleclt_0031_guard_ok = (
  ...
  AND @settleclt_0031_confirm_writes_stopped = 'WRITES_STOPPED'
  AND @settleclt_0031_confirm_backup_verified = 'BACKUP_VERIFIED'
  AND @settleclt_0031_confirm_journal_absent = 'JOURNAL_ABSENT'
...
WHERE u.newsletterOptIn <> IF(n.id IS NULL, false, true);
...
SET @settleclt_0031_confirmation_index_sql = IF(
  @settleclt_0031_confirmation_index_rows = 0,
...
SET @settleclt_0031_unsubscribe_index_sql = IF(
  @settleclt_0031_unsubscribe_index_rows = 0,
...
SET @settleclt_0031_final_column_count = (
  ...
  WHERE TABLE_SCHEMA = DATABASE()
    AND TABLE_NAME = 'newsletter_subscribers'
    AND COLUMN_NAME IN ('source', 'consentVersion', 'consentedAt', 'unsubscribeTokenHash')
    AND IS_NULLABLE = 'NO'
);
...
SET @settleclt_0031_final_guard_sql = IF(
  @settleclt_0031_final_column_count = 4 AND @settleclt_0031_final_index_count = 2,
