...
const consent = fs.readFileSync(path.join(root, "client/src/components/CookieConsent.tsx"), "utf8");
const main = fs.readFileSync(path.join(root, "client/src/main.tsx"), "utf8");
const mixpanel = fs.readFileSync(path.join(root, "client/src/lib/mixpanel.ts"), "utf8");
...
describe("analytics consent contracts", () => {
  ...
  it("gates Mixpanel operations on accepted consent", () => {
    expect(mixpanel).toContain("localStorage.getItem(CONSENT_KEY)");
