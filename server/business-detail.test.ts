...
describe("Sitemap Business URLs", () => {
  it("does not expose the confirmed unavailable directory destinations", () => {
    const unavailableHosts = [
      "adventcoworking.com",
      "asihvac.com",
      "arborproclt.com",
      "awazeethiopiancuisine.com",
      "bakusouthpark.com",
      "baotimeclt.com",
      "anytimeplumbing.com",
    ];
    ...
    for (const service of SERVICES) {
      expect(
        unavailableHosts.some(host => service.website.includes(host))
      ).toBe(false);
    }
  });

  it("should generate 700+ unique business slugs for sitemap", () => {
    const slugs = Array.from(new Set(SERVICES.map(s => toSlug(s.name))));
